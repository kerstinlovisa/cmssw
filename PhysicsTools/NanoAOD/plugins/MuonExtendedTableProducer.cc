...
void MuonExtendedTableProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  std::vector<int> numberOfMatchedStation, numberOfValidPixelHits, numberOfValidTrackerHits, numberInnerHitsMissing,
      trackerLayersWithMeasurement, numberInnerHits;
  std::vector<float> relIso0p4;
  ...
  for (unsigned int i = 0; i < nMuons; i++) {
    ...
    fillLeptonJetVariables(
        &muon, jetHandle, pv, *rhoHandle, &jetIdx, relIso0p4, &jetPtRatio, &jetPtRel, &jetSelectedChargedMultiplicity);
    ...
    numberInnerHitsMissing.push_back(!muon.innerTrack().isNull() ? muon.innerTrack()->hitPattern().numberOfLostHits(
                                                                 ...
                                                                 : 0);
    trackerLayersWithMeasurement.push_back(
        (!muon.innerTrack().isNull()) ? muon.innerTrack()->hitPattern().trackerLayersWithMeasurement() : 0);
    numberInnerHits.push_back(
        (!muon.globalTrack().isNull())
            ? muon.globalTrack()->hitPattern().numberOfValidMuonHits()
            : (!muon.outerTrack().isNull() ? muon.outerTrack()->hitPattern().numberOfValidMuonHits() : 0));
...
int MuonExtendedTableProducer::findMatchedJet(const reco::Candidate& lepton,
                                              const edm::Handle<std::vector<pat::Jet>>& jets) const {
  int iJet = -1;
